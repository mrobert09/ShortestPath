...
def find_adjacent(point, path_dict, adjacent_points, max_x, max_y):
    for x in range(-1, 2):
        for y in range(-1, 2):
...
def generate_path_dictionary(starting_point, max_x, max_y, blocked_points):
    path_dict = {}
    ...
    find_adjacent(starting_point, path_dict, adjacent_points, max_x, max_y)
    ...
    while not adjacent_points.empty():
        ...
        find_adjacent(next_point[0], path_dict, adjacent_points, max_x, max_y)
...
def find_distance(start_point, end_point, path_dict, path=None, total=0):
    if path == None:
        path = []
    ...
    return find_distance(start_point, prev_point, path_dict, path, total)
...
def main():
    ...
    blocked_points = generate_blocked()
    path_dict = generate_path_dictionary(starting_point, max_grid_x, max_grid_y, blocked_points)


    distance, path = find_distance(starting_point, ending_point, path_dict)
    print(distance)
    print(path)
